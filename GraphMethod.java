...
public class GraphMethod {
	...
	public GraphMethod(ArrayList<String>lines) {
		this.lines = lines;
	}
	...
	public int[][] buildGraph(){
 		Node root = new Node(GraphMethod.nodeName,0);
		GraphMethod.nodeName++;
		...
		while(true){
			...
			if(this.lines.get(lineNo).contains("else if")){
				...
				if(statement_node == null){
					statement_node = new Node(GraphMethod.nodeName, lineNo);
					GraphMethod.nodeName +=1;
					if(!lastNodesInBranch.isEmpty()){
